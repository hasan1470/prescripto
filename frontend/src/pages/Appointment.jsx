...
const getAvailableSlots = async () => {
  ...
  let today = new Date()
  let daysAdded = 0
  let offset = 0 // how many days from today we’re checking
  ...
  while (daysAdded < 7) {
    ...
    endTime.setDate(today.getDate() + offset)
    endTime.setHours(21, 0, 0, 0)

    // Skip current date if already past last slot time
    if (offset === 0 && today >= endTime) {
      offset++
      continue
    }
    ...
    <div>
      ...
      <div className='flex flex-col sm:flex-row gap-4'>
        ...
        <div className='flex-1 border border-gray-300 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
          ...
          <p className='text-gray-600 font-medium mt-4'>Appoinment Fee: <span className='text-gray-800'>{currencySymbol[0]}{docInfo.fees}</span></p>
        </div>
      </div>
      ...
      <div className='sm:ml-72 sm:pl-4 mt-8 font-medium text-gray-700'>
        ...
        <button onClick={bookAppointment} class="bg-primary text-white text-sm font-light px-20 py-3 rounded-full my-6 cursor-pointer">Book an appointment</button>
